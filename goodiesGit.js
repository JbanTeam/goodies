...
// git config --global core.editor "subl -n -w" - для линукса, sublime
// git config --global core.editor "code -w" - для линукса, vs code
...
// git reset --hard - можно использовать чтобы сбросить в рабочей области все незакомиченные изменения(перезапишет файлы в последнее закоммиченное состояние), индекс также сбросится
...
// git balme index.html - кто какую написал строку в файле
...
// git merge feature - слияние текущей ветки с веткой feature, если ветка feature отходит от ветки master, и у ветки master нет дальнейших коммитов, то происходит слияние перемоткой
// git merge --no-ff --no-edit feature - флаг --no-ff заставляет merge вместо перемотки делать коммит слияния(лучше, так как видна история, какой ветке какой коммит принадлежал)
...
//* после разрешения конфликта, добавляем конфликтный файл в индекс, так как индексе тоже присутствует конфликт
...
// git merge --abort - прекращает слияние, не затрагивает изменения которые есть только в рабочей директории
...
// git merge --squash 3rdBranch - берет изменения из ветки 3rdBranch и добавляет их в рабочую директорию и индекс, без привязки к ветке с которой были взяты изменения(3rdBranch)
// git commit -m 'Run sayHi and sayBye' - коммитит эти изменения в текущую ветку (master), а ветку 3rdBranch можно спокойно удалить, используется когда история коммитов запутанная и мы не хотим ее кому то показывать
...
// git cherry master feature - показывает эквивалентные коммиты, (-v) выведет название коммитов
...
//* ReReRe - авторазрешение повторных конфликтов, при отмене rebase, merge**********************************************
...
// code . - вызвать редактор(vs code) в и открыть там текущий каталог
// ctrl + shift + c - вызвать терминал в открытом в редакторе каталоге(работает в vs code в линуксе)
// man "команда" - вызывает справку
// "команда" --help - вызывает справку

// sudo mount -o remount,rw /dev/sda5 - ремоунт диска, с правами записи
// curl wttr.in/Новгород - показывает погоду
// sudo ln -s /opt/telegram/Telegram /usr/local/bin/telegram - создается символическая ссылка, теперь можно вызывать телеграмм а консоли telegram
// telegram & - запуск телеграм в фоне, чтобы терминал не подвисал
// alias tg='/opt/telegram/Telegram' - запуск телеграмм tg
